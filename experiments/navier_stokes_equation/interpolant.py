...
class Interpolant:
    ...
    def compute_zt_flowdas(self, D): ## here use new because in the Dz0 has lookback frames rather than just z0 
    ...
    def compute_target_flowdas(self, D): ## here use new because in the Dz0 has lookback frames rather than just z0 
        z0 = D['z0'][:,-1].unsqueeze(1)
        if len(D['z1'].shape)==3:
            z1 = D['z1'].unsqueeze(1)
        else:
